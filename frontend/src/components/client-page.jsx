...
function ClientPage() {
    ...
    useEffect(() => {
        const f = async () => {
            const response = await axios.get('http://localhost:8080/api/person/' + filmId + '/');
            setData(response.data);
        };
        f();
    }, [filmId]);
    const del = async () => {
        await axios.delete('http://localhost:8080/api/person/' + filmId + '/');
        setTimeout(() => nav('/films/', 500));
    };
    const change = async (event) => {
        const toSend = {}
        for (let elem in data) toSend[elem] = data[elem].toString();
        await axios.post('http://localhost:8080/api/person/', toSend);
    }
    ...
    return (
        <div>
            ...
            {build()}
            {showBtns ? <div><DelButton onClick={del} to={'/films/'}>Удалить клиента</DelButton><br/>
